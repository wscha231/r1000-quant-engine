...
class ControlPlaneTests(unittest.TestCase):
    ...
    def test_candidate_data_queue_is_asset_first_and_a2_cannot_self_promote(self):
        queue={'schema_version':'candidate-data-queue-v1','as_of':self.at(-5),
            'research_only':True,'direct_score_contribution':0,
            'authority':{'selector':False,'er':False,'target':False,'portfolio':False,
                         'broker':False,'orders':False,'promotion':False},
            'items':[{'asset_id':'CRYPTO:BTC-USD','identity_kind':'NON_ISSUER_ASSET',
                'security_id':None,'issuer_id':None,'ticker':None,'state':'DATA_PENDING',
                'membership_reasons':[{'reason_type':'EVENT_RESEARCH_TRIGGER'}],
                'required_channels':['price','news_ir','thesis','valuation','expected_return'],
                'channel_status':{
                    'price':'VERIFIED','news_ir':'VERIFIED',
                    'thesis':'CONSUMER_RECEIPT_MISSING',
                    'valuation':'CONSUMER_RECEIPT_MISSING',
                    'expected_return':'CONSUMER_RECEIPT_MISSING'},
                'next_action':'A3_RESEARCH_REQUIRED','first_seen_at':self.at(-20),
                'last_checked_at':self.at(-10)}]}
        board.validate_candidate_data_queue(queue)
        bad=copy.deepcopy(queue); bad['items'][0]['state']='ER_ELIGIBLE'
        with self.assertRaises(board.ContractError):
            board.validate_candidate_data_queue(bad)
        bad=copy.deepcopy(queue); bad['items'][0]['issuer_id']='ISSUER:FAKE'
        with self.assertRaises(board.ContractError):
            board.validate_candidate_data_queue(bad)
        bad=copy.deepcopy(queue); bad['items'][0]['channel_status'].pop('expected_return')
        with self.assertRaisesRegex(board.ContractError,'candidate_queue_channel_mismatch'):
            board.validate_candidate_data_queue(bad)
        bad=copy.deepcopy(queue); bad['items'][0]['last_checked_at']=self.at(5)
        with self.assertRaisesRegex(board.ContractError,'candidate_queue_time_order'):
            board.validate_candidate_data_queue(bad)
