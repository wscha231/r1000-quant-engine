...
SPEC = importlib.util.spec_from_file_location(
    "phase2a", ROOT / "tools" / "run_phase2a_whole_equity_er_research.py"
)
...
def contract():
    return {
        "schema_version": "phase2a-whole-equity-er-research-contract-v2",
...
class Phase2A(unittest.TestCase):
    ...
    def test_full_verified_cohort_maps_existing_21_63_126_outputs(self):
        ...
        self.assertTrue(out["whole_equity_gross_research_er_ready_1_3_6m"])
        ...
        row = out["rows"][0]
        self.assertAlmostEqual(row["gross_research_expected_return_1m"], 0.01)
        self.assertAlmostEqual(row["gross_research_expected_return_3m"], 0.02)
        self.assertAlmostEqual(row["gross_research_expected_return_6m"], 0.03)
        self.assertAlmostEqual(row["gross_research_benchmark_expected_return_1m"], 0.006)
        self.assertAlmostEqual(row["gross_research_expected_alpha_6m"], 0.012)
        ...
        self.assertEqual(out["gross_research_expected_alpha_basis"], "GROSS_BENCHMARK_EXCESS_RESEARCH_NOT_AFTER_COSTS")
        ...
        self.assertIsNone(row["gross_research_expected_return_12m"])
        self.assertFalse(out["global_ranking_ready"])
        self.assertFalse(out["a5_execution_allowed"])
        self.assertEqual(out["artifact_kind"], "GROSS_RESEARCH_ER_EVIDENCE")
        self.assertEqual(
            out["validation_status"],
            "RESEARCH_FIT_OUTPUT_NOT_A3_WALK_FORWARD_VALIDATED",
        )
        self.assertFalse(out["net_of_costs"])
        self.assertFalse(out["a3_validated_er_eligible"])
        self.assertEqual(
            out["er_promotion_gate_status"], "BLOCKED_GROSS_RESEARCH_ONLY"
        )
        self.assertIn("BLOCKED_NOT_NET_OF_COSTS", out["promotion_blockers"])

    def test_canonical_expected_alpha_is_benchmark_excess_not_mixed_challenger_alpha(self):
        out = MOD.run(self.args())
        row = out["rows"][0]
        self.assertAlmostEqual(row["gross_research_expected_alpha_3m"], 0.008)
        self.assertAlmostEqual(row["raw_challenger_expected_alpha_3m"], 0.006)
        self.assertNotEqual(row["gross_research_expected_alpha_3m"], row["raw_challenger_expected_alpha_3m"])
        self.assertEqual(out["gross_research_expected_alpha_basis"], "GROSS_BENCHMARK_EXCESS_RESEARCH_NOT_AFTER_COSTS")
    ...
    def test_12m_is_never_synthesized_from_6m(self):
        ...
        for row in out["rows"]:
            self.assertIsNone(row["gross_research_expected_return_12m"])
            self.assertIsNone(row["gross_research_expected_alpha_12m"])
            self.assertIsNone(row["gross_research_benchmark_expected_return_12m"])
            self.assertEqual(row["horizon_status"]["12m"], MOD.TWELVE_MONTH_BLOCKER)
    ...
    def test_partial_proposal_preserves_missing_security_and_blocks_readiness(self):
        ...
        self.assertFalse(out["whole_equity_gross_research_er_ready_1_3_6m"])
        ...
        self.assertIsNone(missing["gross_research_expected_return_6m"])
    ...
    def test_corporate_action_basis_missing_fail_closes_row(self):
        ...
        self.assertIsNone(row["gross_research_expected_return_1m"])
        self.assertIsNone(row["gross_research_expected_return_3m"])
        self.assertIsNone(row["gross_research_expected_return_6m"])
    ...
    def test_one_horizon_failure_does_not_erase_other_validated_horizons(self):
        ...
        row = next(row for row in out["rows"] if row["ticker"] == "BBB")
        self.assertIsNone(row["gross_research_expected_return_1m"])
        self.assertAlmostEqual(row["gross_research_expected_return_3m"], 0.04)
        self.assertAlmostEqual(row["gross_research_expected_return_6m"], 0.06)
        self.assertEqual(row["status"], MOD.ROW_PARTIAL)
