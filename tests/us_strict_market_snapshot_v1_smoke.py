...
from research.a3_candidate_packet_v1 import A3CandidatePacketError, _validate_market
...
def test_source_bundle_binds_exact_provider_bytes() -> None:
    raw_page = b'{"bars":{"AVGO":[{"t":"2026-09-18T20:00:00Z","c":100.0,"v":1}]}}'
    split_page = b'{"bars":{"AVGO":[{"t":"2026-09-18T20:00:00Z","c":100.0,"v":1}]}}'
    raw_receipt = [{
        "adjustment": "raw",
        "page": 1,
        "sha256": hashlib.sha256(raw_page).hexdigest(),
        "bytes": len(raw_page),
        "endpoint": "https://data.alpaca.markets/v2/stocks/bars",
        "request": {"symbols": "AVGO,SPY", "adjustment": "raw"},
        "page_token_used": False,
    }]
    split_receipt = [{
        "adjustment": "split",
        "page": 1,
        "sha256": hashlib.sha256(split_page).hexdigest(),
        "bytes": len(split_page),
        "endpoint": "https://data.alpaca.markets/v2/stocks/bars",
        "request": {"symbols": "AVGO,SPY", "adjustment": "split"},
        "page_token_used": False,
    }]
    artifact_id, bundle = build_source_bundle(
        session_date="2026-09-18",
        source_identity="ALPACA_V2_IEX_RAW_SPLIT_CONCORDANCE_V1",
        registry_sha256="a" * 64,
        raw_receipts=raw_receipt,
        raw_pages=[raw_page],
        split_receipts=split_receipt,
        split_pages=[split_page],
    )
    payload = json.loads(bundle)
    assert base64.b64decode(payload["bases"][0]["pages"][0]["body_base64"]) == raw_page
    assert base64.b64decode(payload["bases"][1]["pages"][0]["body_base64"]) == split_page

    kwargs = base_kwargs()
    kwargs["raw_artifact_id"] = artifact_id
    kwargs["raw_sha256"] = hashlib.sha256(bundle).hexdigest()
    asset = frame(0.0014)
    bench = frame(0.0006)
    out = compute_snapshot(asset, asset.copy(), bench, bench.copy(), **kwargs)

    _validate_market(
        out,
        datetime(2026, 9, 19, 2, 0, tzinfo=timezone.utc),
        lambda wanted_id, wanted_sha: bundle,
    )
    try:
        _validate_market(
            out,
            datetime(2026, 9, 19, 2, 0, tzinfo=timezone.utc),
            lambda wanted_id, wanted_sha: bundle + b"x",
        )
    except A3CandidatePacketError as exc:
        assert "market_raw_hash_mismatch" in str(exc)
    else:
        raise AssertionError("tampered source bundle was accepted")
...
if __name__ == "__main__":
    ...
    test_source_bundle_binds_exact_provider_bytes()
