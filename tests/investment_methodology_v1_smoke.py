...
def packet() -> dict:
    ...
    for i, name in enumerate(PILLARS):
        pillars[name] = {
            ...
            "peer_relative_assessment": 0.45 + 0.04 * i,
            ...
            "peer_snapshot_sha256": sha(f"peer-{i}"),
...
class InvestmentMethodologyV1Smoke(unittest.TestCase):
    def test_complete_packet_emits_equal_pillar_scores_and_zero_authority(self):
        value = packet()
        result = evaluate_packet(value, "2026-09-19T02:00:00Z")
        ...
        self.assertAlmostEqual(
            ...
            sum(0.45 + 0.04 * i for i in range(10)) / 10,
        )
    ...
    def test_sector_profile_changes_interpretation_not_equal_weights(self):
        first = packet()
        first_result = evaluate_packet(first, "2026-09-19T02:00:00Z")
        second = copy.deepcopy(first)
        second["sector_profile"] = "BIOTECH_PRECOMMERCIAL"
        second_result = evaluate_packet(second, "2026-09-19T02:00:00Z")
    ...
    def test_confidence_is_reported_separately_not_hidden_weight(self):
        value = packet()
        baseline = evaluate_packet(value, "2026-09-19T02:00:00Z")["equal_absolute_score"]
        for name in PILLARS:
    ...
    def test_absolute_and_peer_assessment_are_not_blended(self):
        ...
        for name in PILLARS:
            ...
            value["pillars"][name]["peer_relative_assessment"] = 0.2
        result = evaluate_packet(value, "2026-09-19T02:00:00Z")
    ...
    def test_method_lenses_reuse_canonical_pillars_without_double_counting(self):
        value = packet()
        before = evaluate_packet(value, "2026-09-19T02:00:00Z")
        changed_pillar = "valuation_margin_of_safety"
        value["pillars"][changed_pillar]["absolute_assessment"] = 0.0
        after = evaluate_packet(value, "2026-09-19T02:00:00Z")
        touched = {
    ...
    def test_packet_digest_is_deterministic(self):
        value = packet()
        first = evaluate_packet(value, "2026-09-19T02:00:00Z")["packet_sha256"]
        second = evaluate_packet(copy.deepcopy(value), "2026-09-19T02:00:00Z")["packet_sha256"]
        self.assertEqual(first, second)
        value["pillars"][PILLARS[0]]["absolute_rationale"] += " changed"
        self.assertNotEqual(
            first,
            evaluate_packet(value, "2026-09-19T02:00:00Z")["packet_sha256"],
        )
