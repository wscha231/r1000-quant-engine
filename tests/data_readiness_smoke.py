...
def test_data_readiness_blocks_future_price_manifest() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        latest = root / "outputs"
        reports = latest / "reports"
        ...
        write_json(
            free / "prices" / "replay_price_cache_manifest.json",
            {"start": "2019-01-01", "end": "2099-01-02", "ticker_count": 3, "failed_count": 0, "status": "completed"},
        )
        (free / "sec").mkdir(parents=True)
        (free / "sec" / "companyfacts.zip").write_bytes(b"zip")
        (root / "data_pit" / "macro").mkdir(parents=True)
        ...
        assert any("price cache manifest end date" in item and "after audit date" in item for item in payload["blockers"])
...
def test_data_readiness_blocks_invalid_universe_health() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        latest = root / "outputs"
        reports = latest / "reports"
