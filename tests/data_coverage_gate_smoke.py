...
def test_dead_etf_fails() -> None:
    sec = {"coverage_etf_ratio": 0.0, "coverage_ratio": 0.12, "coverage_13f_ratio": 0.78, "coverage_smart_money_ratio": 0.73}
    ...
    # etf (0.0) and available_from (0.12) both below floor 0.30 -> FAIL
    assert "etf" in fails and "available_from" in fails, fails
...
def test_warn_only_downgrades() -> None:
    sec = {"coverage_etf_ratio": 0.0, "coverage_ratio": 0.12, "coverage_13f_ratio": 0.78, "coverage_smart_money_ratio": 0.73}
    r = dcg.evaluate(sec_enriched=sec, readiness={}, floors=dcg.DEFAULT_FLOORS, warn_only={"etf", "available_from"})
...
def test_all_healthy_passes() -> None:
    sec = {"coverage_etf_ratio": 0.55, "coverage_ratio": 0.60, "coverage_13f_ratio": 0.80, "coverage_smart_money_ratio": 0.75}
...
def test_future_available_from_hard_fails() -> None:
    sec = {"coverage_etf_ratio": 0.9, "coverage_ratio": 0.9, "coverage_13f_ratio": 0.9, "coverage_smart_money_ratio": 0.9}
...
def test_missing_key_treated_as_below_floor() -> None:
    ...
    assert "etf" in absent and "available_from" in absent and "smart_money" in absent, absent
