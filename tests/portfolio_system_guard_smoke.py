...
def test_portfolio_system_guard_cli_targets_are_diagnostic_only() -> None:
    metrics = {
        "_metric_source": "broker_ledger_next_close", "metric_mode": "broker_ledger_next_close",
        "status": "completed",
...
def test_portfolio_system_guard_mission_boundaries_and_missing_metrics() -> None:
    for name, cagr, mdd, expected in [
        ...
        metrics = {"_metric_source": "broker_ledger_next_close", "metric_mode": "broker_ledger_next_close", "status": "completed",
                   "valid_for_production": True, "cagr": cagr, "max_dd": mdd}
        assert portfolio_status(name, metrics, .30, -.28)["target_pass"] is expected
    metrics = {"_metric_source": "broker_ledger_next_close", "metric_mode": "broker_ledger_next_close", "valid_for_production": False,
