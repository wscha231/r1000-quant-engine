...
def main() -> int:
    with tempfile.TemporaryDirectory() as fixture_tmp:
        fixture_out = Path(fixture_tmp) / "goal"
        subprocess.run(
            [
                sys.executable,
                str(REPO_ROOT / "tools" / "run_portfolio_goal_search.py"),
                "--latest-run",
                str(REPO_ROOT / "tests" / "fixtures" / "run287_canonical_baseline"),
                "--output-dir",
                str(fixture_out),
            ],
            cwd=REPO_ROOT,
        ...
        out = fixture_out / "goal_search_summary.json"
        ...
        report = fixture_out / "goal_search_report.md"
