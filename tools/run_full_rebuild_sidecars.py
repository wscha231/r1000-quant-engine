...
run_universe_recovery_candidate_readiness() {
  echo "[universe-recovery] validating review-only fallback universe candidate readiness"
  mkdir -p outputs/full_rebuild_logs
  python tools/check_universe_recovery_candidate_readiness.py \
    --latest-run outputs \
    --price-cache cache_prices \
    --output-dir outputs/universe_recovery_candidate_readiness \
    --min-r1000-base 400 \
    2>&1 | tee outputs/full_rebuild_logs/universe_recovery_candidate_readiness.log || true
}

run_evidence_policy() {
  echo "[evidence-policy] classifying evidence tier and allowed uses"
  mkdir -p outputs/full_rebuild_logs
  python tools/evidence_policy.py \
    --latest-run outputs \
    --user-current-dir outputs/user_current \
    --output-dir outputs/evidence_policy \
    2>&1 | tee outputs/full_rebuild_logs/evidence_policy.log || true
}

run_clean_7y_research_readiness() {
  echo "[clean-7y] checking Alpha Plane research readiness"
  mkdir -p outputs/full_rebuild_logs
  python tools/check_clean_7y_research_readiness.py \
    --latest-run outputs \
    --user-current-dir outputs/user_current \
    --output-dir outputs/clean_7y_research_readiness \
...
run_proxy_10y_robustness() {
  ...
  mkdir -p outputs/full_rebuild_logs
  python tools/check_proxy_10y_robustness.py \
    --latest-run outputs \
    --output-dir outputs/proxy_10y_robustness \
