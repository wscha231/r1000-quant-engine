...
from tools.subscription_manager.history_adapter import read_pinned
...
def run(events_path: Path, events_hash: str, output: Path, expected_head: str,
    ...
    # The history input stage authenticates archive bytes and preserves coverage.
    # Quote/calendar/review/delivery adapters are still a separate prerequisite.
    if envelope['data_kind'] == 'REAL':
        raise ValueError('real_model_event_adapters_not_integrated')
...
def main():
    if sys.argv[1:2] == ['prepare-history']:
        from tools.subscription_manager.history_adapter import main as prepare
