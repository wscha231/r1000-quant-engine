...
def parse_csv_field(name: str, text: str) -> Any:
    value = _parse_scalar(text)
    # CSV has no native numeric type; only this declared integer field is decoded.
    if name == "event_version" and isinstance(value, str) and value.isascii() and value.isdigit():
        return int(value)
    return value
...
def load_rows(path: Path) -> list[dict[str, Any]]:
    ...
    if suffix == ".jsonl":
        ...
        return rows
    if suffix == ".json":
        payload = checked_json(path.read_text(encoding="utf-8"))
    ...
    if suffix == ".csv":
        with path.open("r", encoding="utf-8", newline="") as handle:
            return [
                {k: parse_csv_field(k, v) for k, v in row.items()}
...
def merge_immutable_ledger(existing: list[dict[str, Any]], incoming: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return merge_immutable_events(existing, incoming)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--as-of", default=None, help="UTC feature decision cutoff; not live issuance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if existing_path is not None:
        if not existing_path.is_file():
            raise ContractError("explicit existing ledger is missing")
