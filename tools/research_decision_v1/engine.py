...
def validate_config(config):
    ...
    us_only=config.get('market_profile')=='US_LISTED_USD_V1'
    if config.get('market_profile') not in (None,'US_LISTED_USD_V1'):raise ValueError('unknown_market_profile')
    if config['target_counts'] != ({'US':5,'KR':0} if us_only else {'US':5,'KR':2}):raise ValueError('target_count_contract_mismatch')
    if us_only and config['country_caps'] != {'US':1.,'KR':0.}:raise ValueError('us_market_cap_contract')
...
def run_decisions(exports, context, config, previous=None, *, quality_bundle=None):
    ...
    readiness['target_scope_complete']=readiness['target_5_plus_2_complete']
    if config.get('market_profile')=='US_LISTED_USD_V1':readiness.pop('target_5_plus_2_complete')
