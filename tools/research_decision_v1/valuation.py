...
def evaluate_security(security, config, cutoff):
    ...
    try:
        ...
        for field, error in (("target_price", "scenario_target_order_invalid"), ("total_return", "scenario_return_order_invalid")):
            values = [number(s[field]) for s in valued]
            if any(a > b and not math.isclose(a, b, rel_tol=1e-12, abs_tol=1e-12) for a, b in zip(values, values[1:])):
        ...
        dispersion = max(s["total_return"] for s in valued)-min(s["total_return"] for s in valued)
        utility = net - config["downside_penalty"] * max(0., -bear["total_return"]) - config["dispersion_penalty"] * dispersion
        ...
        row.update(valuation_ready=True, probability_type="SUBJECTIVE_SCENARIO", method=p["method"],
                   ...
                   uncertainty=risk["uncertainty"], scenario_range=dispersion,
