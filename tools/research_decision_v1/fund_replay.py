...
def scalar_evidence(record, cutoff, *, positive=False, evidence_mode='CONTEMPORANEOUS_RECORDS'):
    source_url(record["source"])
    published, observed = timestamp(record["published_at"]), timestamp(record["observed_at"])
    require(published <= observed and published <= timestamp(cutoff), "fund_scalar_future_or_reversed")
    if evidence_mode=='RETROSPECTIVE_RECONSTRUCTION' and observed>timestamp(cutoff):
        version=record.get('archive_version',{})
        source_url(version.get('source'))
        require(version.get('kind')=='ORIGINAL_RELEASE' and version.get('published_at')==record['published_at'] and
                version.get('retrieved_at')==record['observed_at'] and
                isinstance(version.get('raw_sha256'),str) and len(version['raw_sha256'])==64 and
                set(version['raw_sha256'])<=set('0123456789abcdef') and observed<=datetime.now(timezone.utc),
                'fund_scalar_archive_version')
    else:require(observed<=timestamp(cutoff),'fund_scalar_future_or_reversed')
...
class Fund:
    def __init__(self, manifest, config, decision_sink=None):
        ...
        if config.get('market_profile')=='US_LISTED_USD_V1':
            require(self.markets==['US'] and manifest['fx_conversion_bps']==0.,'fund_us_profile_scope')
        ...
        self.rate = scalar_evidence(manifest["opening_risk_free"], self.start.isoformat(),evidence_mode=manifest['evidence_mode'])
    ...
    def close(self, event, *, seed=False):
        ...
        if "KR" in self.markets or event.get("fx") is not None:
            new_fx = scalar_evidence(event["fx"], event["time"], positive=True,evidence_mode=self.spec['evidence_mode'])
        ...
        if not seed:
            self.rate = scalar_evidence(event["risk_free"], event["time"],evidence_mode=self.spec['evidence_mode'])
            require(self.rate > -1, "fund_rate_domain")
    ...
    def decide(self, event, packet):
        ...
        for observation in packet["macro"]:
            scalar_evidence(observation,event["time"],evidence_mode=self.spec['evidence_mode'])
