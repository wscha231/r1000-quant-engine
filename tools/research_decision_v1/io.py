...
from tools.research_decision_v1.platform_io import input_descriptor, is_redirect, publish_staged, sync_directory
...
def immutable_bytes(path, data):
    ...
    try:
        ...
        try:
            publish_staged(temporary, path)  # Atomic, no replacement on both platforms.
        except FileExistsError:
            try:
                if is_redirect(path) or read_bytes(path, maximum=len(data)) != data: raise ValueError("immutable_history_conflict")
            except ValueError as exc: raise ValueError("immutable_history_conflict") from exc
