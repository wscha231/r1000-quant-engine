...
def _propose_once(rows, securities, context, config, common_errors, cutoff, excluded_new):
    ...
    if prior:
        ...
        for sid, old in prior.items():
            ...
            if desired < old:
                ...
                benefit = max((replacement_net_improvement(row, r, config) for r in alternatives), default=0.)
                if benefit <= config["replacement_improvement_buffer"]:
