...
def propose(rows, securities, context, config, common_errors, cutoff):
    ...
    if prior:
        ...
        def fit_new_capacity():
            # Retained positions consume budget, count and risk room first.
            new_ids = [sid for sid in weights if sid not in prior and weights[sid] > 0]
            existing_gross = sum(weights.get(sid, 0.) for sid in prior)
            new_gross = sum(weights[sid] for sid in new_ids)
            scale = max(0., min(1., (budget-existing_gross)/new_gross)) if new_gross else 1.
            for sid in new_ids: weights[sid] *= scale
            for market in ("US", "KR"):
                occupied_ids = [sid for sid in prior if weights.get(sid, 0.) > 0 and securities[sid]["market"] == market]
                members = sorted((sid for sid in new_ids if securities[sid]["market"] == market),
                                 key=lambda sid: (by_id[sid]["investment_rank"], sid))
                slots = max(0, config["target_counts"][market]-len(occupied_ids))
                for sid in members[slots:]:
                    weights[sid] = 0.
                    if "retained_incumbent_count_capacity" not in reasons[sid]: reasons[sid].append("retained_incumbent_count_capacity")
                occupied = sum(weights[sid] for sid in occupied_ids)
                amount = sum(weights[sid] for sid in members)
                factor = max(0., min(1., (config["country_caps"][market]-occupied)/amount)) if amount else 1.
                for sid in members: weights[sid] *= factor
