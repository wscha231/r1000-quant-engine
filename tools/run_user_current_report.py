...
def render_action_summary(
    ...
    lines = [
        ...
        f"- proxy_10y_robustness_pass: `{str(evidence.get('proxy_10y_robustness_pass', False)).lower()}`",
        f"- proxy_10y_reasons: `{', '.join(str(item) for item in evidence.get('proxy_10y_reasons') or [])}`",
    ...
    payload.update(
        {
            ...
            "proxy_10y_reasons": evidence.get("proxy_10y_reasons"),
