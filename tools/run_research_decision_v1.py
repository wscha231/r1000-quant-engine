...
from tools.research_decision_v1.io import immutable_json, read_json, research_root, source_code_hash
...
def main():
    ...
    if report_path.is_symlink(): raise ValueError("output_symlink")
    try:
        with report_path.open("xb") as handle: handle.write(report_bytes)
    except FileExistsError:
        if report_path.read_bytes() != report_bytes: raise ValueError("immutable_history_conflict")
