...
def load_terminal_lifecycle_exclusions(
    ...
    *,
    ...
    """Return only terminal events known by the current decision time.

    A stale or delisted symbol may legitimately have no bar on the requested
    session.  It is excluded only when a hash-pinned, exact-acceptance event is
    effective by that session and was public by the decision time.  Missing or
    weak lifecycle evidence therefore remains fail-closed rather than turning
    into a carried-forward price.
    """
    ...
    frame["evidence_status"] = frame["evidence_status"].astype(str).str.strip().str.lower()
    effective = pd.to_datetime(frame["effective_date"], errors="coerce").dt.normalize()
    last_trade = pd.to_datetime(frame["last_trading_date"], errors="coerce").dt.normalize()
    ...
    if structural_bad.any():
        bad = ",".join(frame.loc[structural_bad, "ticker"].replace("", "<blank>").tolist())
    ...
    if active["ticker"].duplicated().any():
        dupes = ",".join(sorted(active.loc[active["ticker"].duplicated(False), "ticker"].unique()))
    ...
    active["available_from"] = available.loc[active.index].dt.strftime("%Y-%m-%dT%H:%M:%SZ")
...
def build(
    ...
    *,
    ...
    if pd.isna(decision_time):
        raise ValueError("--decision-time-utc is required and must include a valid timestamp")
