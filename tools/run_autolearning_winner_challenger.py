...
def build_decision(
    ...
    return {
        ...
        "verdict": decide_verdict(onset, lifecycle, replay_status),
    }


def decide_verdict(onset: dict[str, Any], lifecycle: dict[str, Any], replay_status: dict[str, Any]) -> str:
    if lifecycle.get("status") == "missing":
        return "BLOCKED_MISSING_WINNER_LIFECYCLE"
    if onset.get("status") == "missing":
        return "BLOCKED_MISSING_WINNER_ONSET_STUDY"
...
def render_report(decision: dict[str, Any], event_rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "Research-only harness connecting AutoLearning v2 with winner lifecycle and winner onset studies.",
        "",
        "## Verdict",
        "",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    decision = build_decision(baseline, autolearning, lifecycle, onset, event_rows, replay_status)
