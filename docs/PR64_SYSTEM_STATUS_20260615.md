...
| A/B result verifier | `run_ab_result_verifier.py` compares completed Concentrated recovery A/B runs against a baseline, requires broker-ledger next-close, 8-year validity, target + strengthened gate pass, attribution/system-acceptance evidence, and keeps `production_activation_allowed=false` | review verifier wired |
