...
Publication success includes a bounded byte comparison after publication/sync. A procfs write to an anonymous Linux inode causes `published_bytes_mismatch`; no successful receipt is produced. This is a point-in-time integrity check, not an assertion that a same-UID process can never modify its files later. Windows failure/collision disposal uses the retained handle before close ([FILE_DISPOSITION_INFO](https://learn.microsoft.com/en-us/windows/win32/api/winbase/ns-winbase-file_disposition_info)); successful rename never deletes the old staging pathname.
