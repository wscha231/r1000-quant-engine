...
manifest-declared files, and the already verified immutable-head selection
receipt. It accepts the supplied verifier receipt only when the complete JSON
object and canonical serialized bytes equal the recomputed result. Missing,
reformatted, forged, stale, or hash-mismatched receipts fail closed.
...
| Immutable-head selection | Verifies and selects one linear immutable chain | Unchanged; its exact receipt is bound into the new verifier envelope |
...
| Failure policy | Producer-valid evidence was structurally impossible to accept | Missing or inconsistent raw, files, selection, or receipt blocks |
...
| Path | H1 purpose |
| --- | --- |
| `tools/run287_paper_ledger_integrity.py` | Build and atomically write the deterministic verifier receipt; bind raw manifest/file-set hashes and immutable selection lineage. |
...
| `.github/workflows/daily_operating_selection_refresh.yml` | Generate the read-only receipt before preflight and retain raw manifest, selection receipt, verifier receipt, and verifier log as evidence. |
| `tests/run287_risk_outcome_parent_preflight_smoke.py` | Replace the producer-invalid fake manifest with a real 243-file, six-head-equivalent producer fixture and fail-closed regressions. |
| `tests/workflow_artifact_smoke.py` | Lock receipt ordering, arguments, and evidence-artifact inclusion. |
...
| Check | Result |
| --- | --- |
| H1 producer/preflight smoke | `13/13` passed |
| Workflow artifact/order smoke | passed |
| Focused registered PR validation | `2/2` test files passed |
| Shared-lessons and GitHub-operating-standard smokes | `2/2` passed |
| Repository CI artifact hygiene smoke | `5/5` passed |
...
That is a successful H1 outcome, not authorization to migrate. `READY` remains
impossible on a scheduled event and no authorization flag is exercised by H1.

## Why migration must be a separate workflow

The daily workflow reaches target construction and the transactional paper
ledger before `Freeze accepted risk-outcome parent`. Using its current
`allow_quarantined_legacy_outcome_parent` dispatch input would therefore grant
more authority than the intended one-time parent transition.
