...
- **Status:** under measurement now — lever-sweep daily-stop grid in run
  `27957500268` (`default,-0.12:-0.20,-0.10:-0.15,-0.08:-0.12`).
...
- **Status:** under measurement — conc-gross floor arm `{0.0,0.7,0.8,0.9}` in run
  `27957500268`. Target: recover the conc CAGR gap (−5.65pp) without breaking 2022.
...
1. **(in flight)** Read `outputs/lever_sweep/summary.json` from run `27957500268`
   (now hardened: writes `status`; daily-stop arm persists first). Record Lever 2/3
   effect on conc CAGR/MaxDD.
