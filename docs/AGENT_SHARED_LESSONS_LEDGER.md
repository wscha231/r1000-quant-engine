...
## 2026-08-19 - Current macro can route a shadow sleeve budget without becoming stock alpha

- A current 2026-08-18 macro packet passed with 9/9 market components, 13/13
  FRED components, no critical missing value, and conservative availability
  cutoffs. It is current-decision evidence only because FRED uses the current
  vintage and the market proxies are not historical PIT macro vintages.
- The completed-session evidence did not confirm a broad crisis: SPY remained
  above its 200-day average, VIX and high-yield OAS were contained, and the
  market family was risk-supportive. Liquidity was mildly defensive, while
  inflation and high long yields justified blocking an unvalidated risk-on
  expansion.
- Freeze macro outside stock ranking. The new shadow router uses four equally
  weighted stress families and only tilts an explicitly selected risk profile;
  it has no ticker, score, selector, target, order, or ledger interface.
- The balanced current proposal is 60% risk assets, 25% stability assets
  (20% short Treasury category and 5% intermediate Treasury category), and
  15% cash/broker MMF. It is a shadow control, not personalized advice or
  performance proof, and the accepted BROKER_CASH_OR_MMF default is unchanged.
- Do not call the July FOMC outcome a rate hike: the target range remained
  3.50%-3.75%, though three voters preferred a 25 bp increase. Distinguish an
  actual policy move from hawkish dissent and a market-driven rise in long
  yields.
- The scientific stock-weighting gate remains BLOCKED_DATA_READINESS. Do not
  fit or backtest around missing PIT universe/lifecycle, zero mature decision
  dates, and missing fundamental/13F components; the macro shadow proposal
  does not waive those blockers.
- Full local PR validation passed 222/224 in 775.26 seconds. The two failures
  are the unchanged OHLCV pattern-memory contract CRLF/LF byte-identity issue
  already recorded on the parent stack; do not rewrite that independent
  immutable-chain contract inside this macro allocation feature.
- Do-not-repeat: do not retune rejected P3 re-entry thresholds, use rejected
  BIL P4 evidence, or optimize the new sleeve constants after seeing this
  current outcome. Any historical macro replay must be separately
  preregistered and use PIT macro vintages with exact release timestamps.
- Evidence: `docs/run287_macro_risk_budget_shadow_contract.json`,
  `docs/CODEX_RUN287_MACRO_RISK_BUDGET_SHADOW_RESULT_20260819.md`, and local
  output `H:\codex\run287_macro_risk_budget_shadow_20260818_balanced_v3`
  generated by code commit `4a987ee66b8c50af4ad9a0a8b2bda6ff405f5139`.
- Fullrun executed: false. Historical fit/backtest executed: false. Stock
  selection and target/order/ledger mutation executed: false. Production/live
  trading and automatic promotion enabled: false.
