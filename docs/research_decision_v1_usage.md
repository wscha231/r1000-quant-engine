...
3. `python -I tools/run_research_decision_v1.py --market-export <US-export.json>
...
Second H2 review corrections: the four questions and global admission use KRW
returns; local questions remain separately labeled. Final country slots are
filled after feasibility clipping. All purchases, including strengthened-thesis
ADDs, share remaining cash/risk room after retained holdings. A valid submitted
book is preserved when a holding is absent from the research evidence universe.
Optional evidence value/provenance changes are attributed separately.
The consumer checks and snapshots source bytes with stdlib before research
imports and executes that same snapshot. Artifacts live under
`runs/<decision_hash>/<source_commit>/`, allowing a renderer-only revision to
...
Third H2 review: retained-book capacity clipping retries candidate selection
after dropping an infeasible new name. Replacement comparisons charge each
market's sell/buy leg. Incremental ADD weight requires the entry return hurdle;
the lower hold hurdle applies only to retention. Previous records are checked
for persistable metadata and a closed SHA256 component schema; known output
boolean flags are typed before constructing the sanitizer view. These flags
...
Previous research outputs retain their typed historical ranks only for the
...
H2 runtime boundary follow-up (reviews 3952174939/3952174941/3952174942):
Use `python -I` from an interpreter with the pinned dependencies installed in its own site-packages. The consumer refuses ordinary non-isolated invocation before importing helpers. It replaces inherited import search paths with the verified first-party snapshot and validated stdlib/site-packages locations under the selected interpreter prefixes. `PYTHONPATH`, the original tools directory and injected extra search paths are excluded. The interpreter installation and its third-party packages remain an explicit trust boundary; source manifests do not claim those dependency bytes were verified.
