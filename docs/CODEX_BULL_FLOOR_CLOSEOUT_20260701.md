# Bull-Floor Closeout - 2026-07-01

## Verdict

Broad Concentrated bull-floor / gross-floor is rejected for the run `28436307420` official-book path.

This is negative evidence, not a production promotion result.
...
The harness was checked for the two failure-sensitive conditions:
...
The control arm reproduced the cash-carry baseline, so the rejection is interpretable.

The result shows that Concentrated cash is not simple idle cash. In this strategy, a substantial portion of cash is load-bearing drawdown defense. Broadly forcing stock exposure into the existing small leader set raises idiosyncratic and theme concentration risk enough to reduce CAGR and materially worsen MaxDD.
...
Any future cash redeployment hypothesis must be narrow and must explain why it will avoid the observed drawdown explosion.
