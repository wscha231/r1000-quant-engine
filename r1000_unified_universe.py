...
from r1000_legacy_input_guard import validate_current_frame, InputIntegrityError
...
def reconcile_inventory(scored, universe, features=None, metadata=None, prices=None, rs=None):
    ...
    score_valid = (frame['score'].map(lambda v: _number(v) is not None)
...
def build_unified_scored(scored_csv=DEFAULT_SCORED_CSV, output_csv=DEFAULT_OUTPUT_CSV, verbose=True):
    ...
    _atomic_text(Path(str(path) + '.coverage.json'), json.dumps({
        'status': 'BLOCKED_BUILD_IN_PROGRESS', 'investment_approved': False}))
    ...
    try:
        compatible = validate_current_frame(compatible)
    except InputIntegrityError as exc:
        summary['status'] = 'BLOCKED_INPUT_INTEGRITY'
