...
def build_walk_forward_impact_estimates(
    ...
    *,
    ...
    actual_index = {
        (
            str(r.get("economic_event_id")),
            int(r.get("checkpoint", -1)),
            int(r.get("horizon", -1)),
        ): r
        for r in outcomes
    }
    ...
    for row in checkpoints:
        ...
        for horizon in HORIZONS:
            prior = [
                r
                for r in outcomes
                if int(r.get("checkpoint", -1)) == int(row["checkpoint"])
    ...
    for checkpoint in CHECKPOINTS:
        for horizon in HORIZONS:
            group = [
                r
                for r in resolved
                if int(r.get("checkpoint", -1)) == checkpoint
                and int(r.get("horizon", -1)) == horizon
            ]
            if not group:
                continue
