...
from .runtime import HORIZONS, prediction_key, checkpoint_key, unique_index
...
def build_top_event_impact_outlook(
    ...
    ticker.
    """
    index = unique_index(estimate_rows, prediction_key, "report estimate")
    top_events = list(top_events)
    unique_index(top_events, checkpoint_key, "report top checkpoint")
    ...
    for event in top_events:
        ...
        for horizon in HORIZONS:
            row = index.get(prediction_key(event, horizon))
            if row is None:
        ...
        out.append(
            {
                ...
                "stable_security_id": event.get("stable_security_id"),
                "event_version": event.get("event_version"),
                "decision_at": event.get("decision_at"),
                "model_version": event.get("model_version"),
                "label_contract": event.get("label_contract"),
