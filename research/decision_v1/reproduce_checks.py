...
def main():
    ...
    for label, counts in [('REAL_PILOT', None), ('SYNTHETIC_3', (2, 1)), ('SYNTHETIC_7', (5, 2))]:
        if counts is None:
            ...
            base = research_root(ROOT) / 'synthetic_inputs' / label
            us_path = immutable_json(base/'us.json', fixture_set('US', counts[0]))
            kr_path = immutable_json(base/'kr.json', fixture_set('KR', counts[1]))
            ctx_path = immutable_json(base/'context.json', context())
            expected = 0
