...
Clean-port implementation note:
- `tools/run_integrated_alpha_crisis_challenger.py` is G1
  `research_only_counterfactual` and cannot be used as promotion evidence.
- `tools/build_crisis_governed_target_books.py` is the G2 bridge. It
  materializes crisis-governed research-only target books from observable
  `daily_features` rows and can optionally call
  `tools/run_broker_ledger_replay.py` so the candidate is measured with
  next-close fills, integer shares, cash ledger, and fees.
