...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| main | 33.15% | 30.00% | 0.00pp | -26.02% | -25.00% | 1.02pp | false |
| concentrated | 46.24% | 50.00% | 3.76pp | -25.82% | -28.00% | 0.00pp | false |
...
- `main`: severity=`ok`, avg_cash=26.70%, latest_cash=15.67%, reasons=none
- `concentrated`: severity=`warn`, avg_cash=42.18%, latest_cash=6.12%, reasons=cash_drag_with_cagr_gap
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| main | `main_rebalance_interval_fixed_interval_I1` | 33.42% | 0.00pp | -16.37% | 0.00pp | true | `target_pass_review` |
| concentrated | `concentrated_grid_N3_score_power_I1` | 48.02% | 1.98pp | -24.88% | 0.00pp | false | `needs_alpha_boost` |
...
- `PASS` data_readiness_ready_for_production_replay: status=warn; ready_for_fullrun=True; ready_for_policy_replay=True; blockers=[]; policy_replay_blockers=[]; warnings=['latest target date 2026-06-24 is after latest observable close 2026-06-23; freshness gate uses observable close', 'dated target snapshot archive is missing for this run']
- `PASS` dataset_coverage_audit_available: outputs/reports/dataset_coverage_audit.json
- `PASS` sec_enriched_candidate_materialized_for_audit: sec_enriched_candidate_present=True; rows_with_smart_money_evidence=34258
- `PASS` alphaops_vnext_uses_sec_enriched_candidate_book: candidate_book=/home/runner/work/r1000-quant-engine/r1000-quant-engine/outputs/sec_enriched_candidate_replay/candidate_replay_book_sec_enriched.csv; rows_with_smart_money_evidence=34258
...
- `PASS` main_target_book_reaches_broker_end: selected_role=operating_target_book; target_book_max=2026-06-23; broker_end=2026-06-23; date_gap_days=0; allowed_lag_days=7; rows=1282; path=/home/runner/work/r1000-quant-engine/r1000-quant-engine/outputs/reports/operating_main_target_book.csv
- `PASS` main_operating_target_book_available: operating_book=/home/runner/work/r1000-quant-engine/r1000-quant-engine/outputs/reports/operating_main_target_book.csv; rows=1282; max_date=2026-06-23
- `WARN` main_historical_research_book_reaches_broker_end: historical_book_max=2026-03-31; broker_end=2026-06-23; rows=2109; operating_book_max=2026-06-23; operating_rows=1282
...
- `PASS` concentrated_target_book_reaches_broker_end: selected_role=operating_target_book; target_book_max=2026-06-23; broker_end=2026-06-23; date_gap_days=0; allowed_lag_days=7; rows=497; path=/home/runner/work/r1000-quant-engine/r1000-quant-engine/outputs/reports/operating_concentrated_target_book.csv
- `PASS` concentrated_operating_target_book_available: operating_book=/home/runner/work/r1000-quant-engine/r1000-quant-engine/outputs/reports/operating_concentrated_target_book.csv; rows=497; max_date=2026-06-23
- `WARN` concentrated_historical_research_book_reaches_broker_end: historical_book_max=2026-03-31; broker_end=2026-06-23; rows=23181; operating_book_max=2026-06-23; operating_rows=497
...
- `PASS` main_cash_position_count_contract: latest_date=2026-06-23; cash=19.19%; stock_count=13; crisis_state=GREEN; violations=[]; preexisting_in_baseline=[]
- `PASS` concentrated_concentration_contract: latest_date=2026-06-23; max_name=SNDK@28.50%; max_industry_group=Tech Hardware & Storage@45.09%; group_source=industry_group; violations=[]; preexisting_in_baseline=[]

## Data Quality Update Plan

- Readiness: ready_for_fullrun=`true`, ready_for_policy_replay=`true`, blockers=[], policy_blockers=[]
- Coverage: sec_enriched_candidate_present=`true`, smart_money_rows=34258, candidate_book=`/home/runner/work/r1000-quant-engine/r1000-quant-engine/outputs/sec_enriched_candidate_replay/candidate_replay_book_sec_enriched.csv`
