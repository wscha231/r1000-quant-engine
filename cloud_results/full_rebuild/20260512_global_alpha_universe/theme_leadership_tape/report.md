...
- `DRAM` Roundhill Memory ETF: theme `memory_semiconductors`, 5d 29.69%, 21d 69.84%, attention 2.44, holdings `MU,SNDK,WDC,STX`
- `XSD` SPDR S&P Semiconductor ETF: theme `semiconductors_equal_weight`, 5d 15.95%, 21d 55.17%, attention 1.71, holdings `AMD,INTC,MU,MRVL,ON,MCHP,LSCC,MPWR,TER,ALAB,CRUS,ONTO`
- `SOXX` iShares Semiconductor ETF: theme `semiconductors_broad`, 5d 15.30%, 21d 37.81%, attention 1.59, holdings `NVDA,AVGO,AMD,MU,INTC,QCOM,MRVL,LRCX,AMAT,KLAC,MCHP,ON,MPWR`
- `SMH` VanEck Semiconductor ETF: theme `semiconductors_broad`, 5d 11.13%, 21d 31.66%, attention 1.19, holdings `NVDA,TSM,AVGO,ASML,AMD,MU,INTC,QCOM,LRCX,AMAT,KLAC,ARM`
- `XBI` SPDR Biotech ETF: theme `biotech_small`, 5d 0.94%, 21d 4.10%, attention -0.04, holdings `EXEL,INSM,CRSP,BEAM,EDIT`
- `ARKK` ARK Innovation ETF: theme `innovation_beta`, 5d 2.30%, 21d 15.50%, attention -0.06, holdings `TSLA,COIN,ROKU,HOOD,CRSP,PATH,PLTR`
- `URA` Global X Uranium ETF: theme `nuclear_uranium`, 5d 4.49%, 21d 12.30%, attention -0.12, holdings `CCJ,UEC,UUUU,LEU,NXE,DNN`
- `XME` SPDR Metals & Mining ETF: theme `metals_mining`, 5d 6.48%, 21d 9.67%, attention -0.20, holdings `MP,FCX,CLF,X,NUE,STLD,AA`
- `NLR` VanEck Uranium and Nuclear ETF: theme `nuclear_power`, 5d 1.53%, 21d 6.51%, attention -0.47, holdings `CEG,BWXT,CCJ,LEU,SMR,OKLO`
- `ITA` iShares U.S. Aerospace & Defense ETF: theme `aerospace_defense`, 5d 5.44%, 21d -1.59%, attention -0.52, holdings `RTX,LMT,NOC,GD,RKLB,KTOS,HWM`

## ETF Look-Through Watchlist

- `MU` via `DRAM`/memory_semiconductors: ETF attention 2.44, ticker score 4.65, 5d 37.73%, in universe `True`
- `SNDK` via `DRAM`/memory_semiconductors: ETF attention 2.44, ticker score 4.48, 5d 31.62%, in universe `True`
- `MU` via `XSD`/semiconductors_equal_weight: ETF attention 1.71, ticker score 4.65, 5d 37.73%, in universe `True`
- `AMD` via `XSD`/semiconductors_equal_weight: ETF attention 1.71, ticker score 4.43, 5d 26.25%, in universe `True`
- `MU` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 4.65, 5d 37.73%, in universe `True`
- `STX` via `DRAM`/memory_semiconductors: ETF attention 2.44, ticker score 2.92, 5d 7.66%, in universe `True`
- `WDC` via `DRAM`/memory_semiconductors: ETF attention 2.44, ticker score 2.79, 5d 11.23%, in universe `True`
- `AMD` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 4.43, 5d 26.25%, in universe `True`
- `QCOM` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 4.20, 5d 23.77%, in universe `True`
- `MU` via `SMH`/semiconductors_broad: ETF attention 1.19, ticker score 4.65, 5d 37.73%, in universe `True`
- `AMD` via `SMH`/semiconductors_broad: ETF attention 1.19, ticker score 4.43, 5d 26.25%, in universe `True`
- `QCOM` via `SMH`/semiconductors_broad: ETF attention 1.19, ticker score 4.20, 5d 23.77%, in universe `True`
- `MRVL` via `XSD`/semiconductors_equal_weight: ETF attention 1.71, ticker score 2.70, 5d 3.14%, in universe `True`
- `MRVL` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 2.70, 5d 3.14%, in universe `True`
- `ON` via `XSD`/semiconductors_equal_weight: ETF attention 1.71, ticker score 2.10, 5d 0.16%, in universe `True`
- `NVDA` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 2.11, 5d 8.44%, in universe `True`
- `ON` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 2.10, 5d 0.16%, in universe `True`
- `MCHP` via `XSD`/semiconductors_equal_weight: ETF attention 1.71, ticker score 1.72, 5d 5.47%, in universe `True`
- `AMAT` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 1.95, 5d 11.92%, in universe `True`
- `AVGO` via `SOXX`/semiconductors_broad: ETF attention 1.59, ticker score 1.90, 5d 2.07%, in universe `True`
